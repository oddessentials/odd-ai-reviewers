...
const DocsViewer = {
  ...
  fileTree: [],
  allowedPaths: [],
  allowedPathMap: new Map(),

  // Build list/map of allowed paths for validation
  buildAllowedPathIndex() {
    const paths = [];
    const collect = (items) => {
    ...
    this.allowedPathMap = new Map(paths.map((path) => [path.toLowerCase(), path]));
  },

  getAllowedPathMatch(path) {
    if (!path) return null;
    return this.allowedPathMap.get(path.toLowerCase()) || null;
  },
  ...
  resolveRelativePath(targetPath, baseFilePath) {
    const normalizedTarget = this.normalizePath(targetPath);
    if (!baseFilePath) return normalizedTarget;

    const baseDir = baseFilePath.split('/').slice(0, -1).join('/');
    const combined = baseDir ? `${baseDir}/${normalizedTarget}` : normalizedTarget;
  ...
  buildTree(items, parentUl, paneId = 'primary') {
    for (const item of items) {
      if (item.type === 'dir') {
        ...
        summary.innerHTML = `${this.icons.folder}<span class="name">${item.name}</span>`;
  ...
  async loadFile(fileId, paneId = 'primary') {
    ...
    const allowedPath = this.getAllowedPathMatch(fileId);
    if (!allowedPath) {
    ...
    // Update state
    if (paneId === 'primary') {
      this.state.currentFile = allowedPath;
    } else {
      this.state.compareFile = allowedPath;
    }

    // Update header
    const filePathSlot = headerDiv.querySelector('.file-path-slot');
    if (filePathSlot) {
      filePathSlot.textContent = allowedPath;
    }
    ...
    // Cache check
    if (this.contentCache.has(allowedPath)) {
      bodyDiv.innerHTML = this.contentCache.get(allowedPath);
      this.attachContentListeners(bodyDiv, paneId, allowedPath);
      return true;
    }
    ...
    try {
      const response = await fetch(`${this.basePath}${allowedPath}`);
      ...
      this.contentCache.set(allowedPath, finalHtml);
  ...
  attachContentListeners(container, paneId, fileId) {
    ...
    container.querySelectorAll('a').forEach((link) => {
      ...
      const resolvedPath = this.resolveRelativePath(href, fileId);
      const matchedFile = this.getAllowedPathMatch(resolvedPath);
    ...
    container.querySelectorAll('img').forEach((img) => {
      ...
      if (src && !src.startsWith('http') && !src.startsWith('data:') && !src.startsWith('/')) {
        const resolvedPath = this.resolveRelativePath(src, fileId);
  ...
  highlightActiveFile(fileId, paneId) {
    ...
    tree.querySelectorAll('.file-link').forEach((link) => {
      ...
      if (link.dataset.fileId?.toLowerCase() === fileId.toLowerCase()) {
    ...
    if (bodyDiv) {
      bodyDiv.innerHTML = `
        <div class="intro">
          ...
          <div class="stats">
            <div class="stat">
              <span class="stat-value">${this.allowedPaths.length}</span>
  ...
  async loadManifest() {
    try {
      ...
      this.buildAllowedPathIndex();
      console.log(`Loaded ${this.allowedPaths.length} documentation files from manifest`);
      return true;
    } catch (error) {
