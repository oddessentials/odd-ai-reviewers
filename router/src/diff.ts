...
export function getDiff(repoPath: string, baseSha: string, headSha: string): DiffSummary {
  try {
    ...
    const diffStat = execSync(`git diff --numstat ${baseSha}...${headSha}`, {
      cwd: repoPath,
      encoding: 'utf-8',
    ...
    // Get file statuses
    const nameStatus = execSync(`git diff --name-status ${baseSha}...${headSha}`, {
      cwd: repoPath,
      encoding: 'utf-8',
    });
    ...
    for (const file of files) {
      if (file.status !== 'deleted') {
        try {
          file.patch = execSync(`git diff ${baseSha}...${headSha} -- "${file.path}"`, {
            cwd: repoPath,
            encoding: 'utf-8',
            maxBuffer: 1024 * 1024,
          });
