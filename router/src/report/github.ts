...
export async function reportToGitHub(
    ...
    try {
        ...
        if (
            ...
            commentId = await postPRComment(octokit, context, sorted, reportingConfig.max_inline_comments);
        }
...
async function createCheckRun(
    ...
    if (config.gating.enabled) {
        if (config.gating.fail_on_severity === 'error' && counts.error > 0) {
            conclusion = 'failure';
        } else if (config.gating.fail_on_severity === 'warning' && (counts.error > 0 || counts.warning > 0)) {
            conclusion = 'failure';
        } else if (config.gating.fail_on_severity === 'info' && (counts.error > 0 || counts.warning > 0 || counts.info > 0)) {
            conclusion = 'failure';
        }
    }
...
async function postPRComment(
    ...
    const inlineFindings = findings.filter((f) => f.line).slice(0, maxInlineComments);
