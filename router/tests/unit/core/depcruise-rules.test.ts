 ...
 * - Test infrastructure (tests/) can import vitest
...
function validateDepcruiseArgs(args: string[]): void {
  for (const arg of args) {
    // Allow paths under router/ (relative paths only, no traversal)
    if (arg.startsWith('router/') && !arg.includes('..')) {
      continue;
    }
...
describe('Dependency Cruiser Rule Validation', () => {
  describe('CANARY: Exception Surface Lock', () => {
    ...
    it('should contain ONLY the locked test infrastructure pattern (^router/tests/)', () => {
      const config = loadDepcruiseConfig();
      const notToDevDep = config.forbidden.find((r) => r.name === 'not-to-dev-dep');
      const pathNot = notToDevDep?.from?.pathNot;
      const patterns = Array.isArray(pathNot) ? pathNot : [pathNot];
    ...
    it('should NOT contain any unapproved broad patterns', () => {
      ...
      // The allowed pattern is ^router/tests/ (fully anchored)
      const forbiddenUnanchoredPatterns = [
        /^__tests__/, // Missing path anchor - would match any __tests__
        /^\/test\//, // Too broad
        /^test\//, // Too broad (note: ^router/tests/ is OK because it's anchored)
      ];
  ...
  describe('CANARY: Cross-Platform Path Normalization', () => {
    it('should match POSIX paths with the test infra pattern', () => {
      const posixPath = 'router/tests/unit/integration/hermetic-setup.ts';
    ...
    it('should match Windows paths after normalization', () => {
      ...
      const windowsPath = 'router\\tests\\unit\\integration\\hermetic-setup.ts';
      const normalizedPath = toPosixPath(windowsPath);
      const regex = new RegExp(ALLOWED_TEST_INFRA_PATTERN);

      // After normalization, the pattern should match
      expect(normalizedPath).toBe('router/tests/unit/integration/hermetic-setup.ts');
      expect(regex.test(normalizedPath)).toBe(true);
    });

    it('should NOT match paths outside tests/ even after normalization', () => {
  ...
  describe('not-to-dev-dep rule behavior', () => {
    depcruiseIt(
      'should allow tests/ to import vitest (test infrastructure)',
      () => {
        const result = runDepcruise(
          'router/src/config.ts --config .dependency-cruiser.cjs --output-type err'
        );

        // Production code should not trigger not-to-dev-dep (no vitest imports)
    ...
    depcruiseIt(
      ...
      () => {
        // Tests are now outside src/ so they're not subject to not-to-dev-dep
