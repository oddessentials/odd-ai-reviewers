...
describe('Local Review Integration Smoke Test', () => {
  describe('T114: End-to-end with real git repo', () => {
    ...
    it('should run dry-run mode successfully on the real repo', async () => {
      const output: string[] = [];
      const deps = {
    ...
    it('should show no changes when comparing HEAD to HEAD', async () => {
      const output: string[] = [];
      const deps = {
        ...createDefaultDependencies(),
        env: mockEnv,
    ...
    it('should support cost-only mode', async () => {
      const output: string[] = [];
      const deps = {
        ...createDefaultDependencies(),
        env: mockEnv,
    ...
    it('should work with existing config even without API credentials in dry-run', async () => {
    ...
    it('should fail gracefully when path is not a git repo', async () => {
      const output: string[] = [];
      const deps = {
        ...createDefaultDependencies(),
        env: mockEnv,
